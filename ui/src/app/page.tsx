...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      await openSavedDocument(documentIdFromQuery);
      if (forcedStepFromQuery) {
        setStep(forcedStepFromQuery);
      }
      router.replace("/", { scroll: false });
    })();
  }, [documentIdFromQuery, forcedStepFromQuery, router]);
              ...
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <div className="rounded-lg border p-3">
                  <Button
                    variant="secondary"
                    className="w-full"
                    onClick={() => setStep(hasMergeSources ? "merge" : "review")}
                    disabled={saving}
                  >
                    Перейти без решения
                  </Button>
              ...
              {sourceKindDetected ? (
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="outline">{sourceKindDetected}</Badge>
                    {sourceKindAuto ? (
                  ...
                  {sourceKindRequiresReview ? (
                    <p className="mt-2 text-xs text-amber-900">
                      Низкая уверенность автоопределения. При необходимости
                      вернитесь на шаг загрузки и выберите тип вручную.
                    </p>
                  ) : null}
                </section>
              ) : null}
